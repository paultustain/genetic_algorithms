...
class Track:
    def __init__(self, index):
        ...
        self.overlay_image = image.load(
            os.path.join("images", f"track{index}-overlay.png")
        )
        with open(os.path.join("images", f"track{index}.json"), "r") as f:
            data = json.load(f)
        ...
        self.map_matrix = [
            ...
            for n in 
            range(0, self.track_image.width * self.track_image.height, self.track_image.width)
        ]
