...
class Evolution:
    ...
    def execute(self, rankable_chromosomes, successes):
        # selection
        ...
        for _ in range(int(reproduction_times)):
            for c1, c2 in itertools.batched(keep_chromosomes, 2):
                ...
                offspring.append(c2[:split_index] + c2[split_index:]) 
        # mutation

        for chromosome in offspring[self.keep_count:]:
            for i in range(len(chromosome)):
                if random.randint(0, 4 + (successes)*2) == 1:
                    chromosome[i] = random.random() * 2 - 1

        assert len(offspring) == self.population_count, "offspring is not population count"
        return offspring